...
**oneMKL is not redistributed here.** It is 200+ MB of Intel-licensed binaries and the ISSL requires
its notice files to ship with it, which is the user's call to make, not a build script's. To enable
the toggle on a machine:
...
2. Copy these next to `FFT.dll` (into `Plugin_FFT/__Plugins__/FFT/`), all the same version:
   ...
   `mkl_tbb_thread.3.dll`, the `mkl_vml_*.3.dll` set, `libimalloc.dll`, and `libiomp5md.dll`.
   ...
   **Only the file names above the `.3` DLLs matter for loading; the rest are shipped so that the
   code path Intel dispatches to exists on whatever CPU the project is opened on.** Kernel DLLs are
   architecture-specific and oneMKL loads its layer and kernels at run time.
3. Open the node's Performance page and turn **FFT Backend** on. The Info DAT row `fft_backend` and
